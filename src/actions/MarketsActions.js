...
const [marketsStore, setMarketsStore] = useMarketsStore();
...
function changeBase(market) {
    _clearBatchTimeouts();
    marketsStore.onChangeBase(market);
    return market;
}

function changeBucketSize(size) {
    marketsStore.onChangeBucketSize(size);
    return size;
}

function getMarketStats(base, quote, refresh = false, errorCallback = null) {
...
function subscribeMarket(base, quote, bucketSize, groupedOrderLimit) {
    /*
    ...
    if (typeof groupedOrderLimit === "undefined") {
        ...
        currentGroupedOrderLimit = groupedOrderLimit;
    }
        ...
        return new Promise((resolve, reject) => {
            Apis.instance()
                ...
                .catch(error => {
                    ...
                    marketsStore.onUnSubscribeMarket({unSub: false, market: subID});
                    reject(error);
                });
        });
    }
...
function cancelLimitOrder(accountID, orderID) {
...
function cancelLimitOrders(accountID, orderIDs, fallbackFeeAssets = "1.3.0") {
    if (__DEV__) {
...
function cancelLimitOrderSuccess(ids) {
    ...
    if (!dispatchCancelTimeout) {
        ...
        dispatchCancelTimeout = setTimeout(() => {
            marketsStore.onCancelLimitOrderSuccess(cancelBatchIDs.toJS());
            dispatchCancelTimeout = null;
...
function closeCallOrderSuccess(orderID) {
    marketsStore.onCloseCallOrderSuccess(orderID);
    return orderID;
}

function callOrderUpdate(order) {
    marketsStore.onCallOrderUpdate(order);
    return order;
}

function feedUpdate(asset) {
    marketsStore.onFeedUpdate(asset);
    return asset;
}

function settleOrderUpdate(asset) {
    Apis.instance()
        .db_api()
        .exec("get_settle_orders", [asset, 100])
        .then(result => {
            marketsStore.onSettleOrderUpdate({
                settles: result
            });
        });
}
...
function getTrackedGroupsConfig() {
    Apis.instance()
        ...
        .then(result => {
            marketsStore.onGetTrackedGroupsConfig({
...
function changeCurrentGroupLimit(groupLimit) {
    marketsStore.onChangeCurrentGroupLimit(groupLimit);
    return groupLimit;
}

function clearMarketStatsInInterval(key) {
...
export {
    ...
    cancelLimitOrder,
    cancelLimitOrders,
    cancelLimitOrderSuccess,
    closeCallOrderSuccess,
    callOrderUpdate,
    feedUpdate,
    settleOrderUpdate,
    toggleStars,
    getTrackedGroupsConfig,
    changeCurrentGroupLimit,
    clearMarketStatsInInterval,
    getMarketStatsInterval
};
